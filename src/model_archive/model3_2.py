...
output_summary_file = "model_evaluation_summary_3_2___1.txt"
...
for track_enum, track_details in TRACK_RACE_DATA.items():
    ...
    for model_name, model_pipeline in models_to_evaluate.items():
        ...
        # 1-Stop Strategies
        min_stint_len_1 = 10 
        for stint1_end_lap in range(min_stint_len_1, RACE_LAPS - min_stint_len_1 + 1, 2):
            ...
            for tyre_class1 in available_tyre_classes_for_simulation:
                for tyre_class2 in available_tyre_classes_for_simulation:
                    ...
                    strategy_name = f"1-Stop: {compound1_display} (L1-{stint1_end_lap}) -> {compound2_display} (L{stint2_start_lap}-{RACE_LAPS})"
        ...
        # 2-Stop Strategies
        min_stint_len_2 = 8 
        for stint1_end_lap in range(min_stint_len_2, RACE_LAPS - 2 * min_stint_len_2 + 1, 2):
            for stint2_end_lap in range(stint1_end_lap + min_stint_len_2, RACE_LAPS - min_stint_len_2 + 1, 2):
                ...
                for tyre_class1 in available_tyre_classes_for_simulation:
                    for tyre_class2 in available_tyre_classes_for_simulation:
                        for tyre_class3 in available_tyre_classes_for_simulation:
                            ...
                            strategy_name = f"2-Stop: {compound1_display} (L1-{stint1_end_lap}) -> {compound2_display} (L{stint2_start_lap}-{stint2_end_lap}) -> {compound3_display} (L{stint3_start_lap}-{RACE_LAPS})"
                            simulated_strategies.append({
                                'name': strategy_name,
                                'stints': [
                                    {'tyre_class': tyre_class1, 'start_lap': 1, 'end_lap': stint1_end_lap},
        ...
        if not current_model_results_df.empty:
            ...
            print(f"\n    Optimal Strategy for {model_name} on {current_track_name}:")
    ...
    with open(output_summary_file, 'a') as f:
        ...
        f.write(f"\n{'='*80}\n\n") # Add a separator for readability between tracks
