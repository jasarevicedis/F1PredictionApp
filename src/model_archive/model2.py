...
if all_laps_df.empty:
    ...
    if model_data.empty:
        ...
        # Define common race strategies to simulate (laps per stint, compounds)
        # This is a simplified set of strategies. More complex strategies could be added.
        # Format: (strategy_name, [(stint1_laps, stint1_compound), (stint2_laps, stint2_compound), ...])

        simulated_strategies = []

        # --- 1-Stop Strategies with similar stint lengths ---
        stint_length_1_stop = SUZUKA_RACE_LAPS // 2

        for compound1 in available_compounds:
            for compound2 in available_compounds:
                # Avoid using the same compound for both stints (unless it's the only option, which is unlikely)
        ...
        # --- 1-Stop Strategies with stint ratio 2/3---
        stint_length_1_stop = 22
