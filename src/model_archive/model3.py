...
output_base_dir = "data/Data_Suzuka"
session_track = Track.SUZUKA

SUZUKA_RACE_LAPS = 53
AVG_PIT_STOP_TIME_SECONDS = 23.5
...
if all_laps_df.empty:
    ...
    if model_data.empty:
        ...
        # handle_unknown='ignore' is important for prediction if a compound not seen in training appears.
        ...
        hypothetical_track_temp = 22.50 # degrees Celsius
        ...
        # --- 1-Stop Strategies with various stint lengths ---
        # Define a range of possible first stint lengths for a 1-stop strategy
        # A reasonable range might be from 1/3 to 2/3 of the race distance
        # Ensure the remaining laps are enough for the second stint (min 1 lap)
        min_stint_length = SUZUKA_RACE_LAPS // 4 # Example: minimum stint length 
        max_first_stint_length = SUZUKA_RACE_LAPS - min_stint_length # Ensure second stint is also at least min_stint_length

        # We can define a set of specific lap numbers for the first pit stop
        # Or, we can iterate through a range with a step
        first_stint_lengths = list(range(min_stint_length, max_first_stint_length + 1, 3)) # Every 3 laps as an example step
        
        # Add common 1-stop strategies like equal stints or 2/3, 1/3 splits if not covered by the range
        if SUZUKA_RACE_LAPS // 2 not in first_stint_lengths:
            first_stint_lengths.append(SUZUKA_RACE_LAPS // 2)
        if 2 * SUZUKA_RACE_LAPS // 3 not in first_stint_lengths:
            first_stint_lengths.append(2 * SUZUKA_RACE_LAPS // 3)
        first_stint_lengths = sorted(list(set(first_stint_lengths))) # Remove duplicates and sort

        print(f"\nGenerating 1-Stop strategies with first stint lengths: {first_stint_lengths}")

        for stint1_len in first_stint_lengths:
            stint2_len = SUZUKA_RACE_LAPS - stint1_len
            if stint2_len <= 0: # Ensure the second stint is valid
                continue
            ...
            for compound1 in available_compounds:
                for compound2 in available_compounds:
                    ...
                    strategy_name = f"1-Stop: {compound1} ({stint1_len} laps) -> {compound2} ({stint2_len} laps)"
                    simulated_strategies.append({
                        'name': strategy_name,
                        'stints': [
                            {'compound': compound1, 'start_lap': 1, 'end_lap': stint1_len},
                            {'compound': compound2, 'start_lap': stint1_len + 1, 'end_lap': SUZUKA_RACE_LAPS}
                        ],
                        'pit_stops': 1
                    })

        # --- 2-Stop Strategies with various stint lengths ---
        # For 2-stop, we need to consider two pit stops, meaning three stints.
        # This becomes combinatorially more complex. We can simplify by
        # defining a range for the first two stint lengths, with the third being the remainder.
        
        # Define possible stint lengths for the first and second stints
        # For simplicity, let's assume each stint should be at least min_stint_length
        # and not exceed a certain percentage of total laps.
        
        min_stint_for_2_stop = SUZUKA_RACE_LAPS // 5 # Example: min 1/5 of race distance per stint
        
        # Iterate over possible first stint lengths
        for stint1_len in range(min_stint_for_2_stop, SUZUKA_RACE_LAPS - 2 * min_stint_for_2_stop + 1, 3):
            # Iterate over possible second stint lengths
            for stint2_len in range(min_stint_for_2_stop, SUZUKA_RACE_LAPS - stint1_len - min_stint_for_2_stop + 1, 3):
                stint3_len = SUZUKA_RACE_LAPS - stint1_len - stint2_len

                if stint3_len <= 0: # Ensure the third stint is valid
                    continue
                    ...
                    for compound2 in available_compounds:
                        for compound3 in available_compounds:
                            if compound1 == compound2 or compound2 == compound3:
                                continue # Avoid identical consecutive compounds

                            strategy_name = f"2-Stop: {compound1} ({stint1_len} laps) -> {compound2} ({stint2_len} laps) -> {compound3} ({stint3_len} laps)"
                            simulated_strategies.append({
                                'name': strategy_name,
                                'stints': [
                                    {'compound': compound1, 'start_lap': 1, 'end_lap': stint1_len},
                                    {'compound': compound2, 'start_lap': stint1_len + 1, 'end_lap': stint1_len + stint2_len},
                                    {'compound': compound3, 'start_lap': stint1_len + stint2_len + 1, 'end_lap': SUZUKA_RACE_LAPS}
                                ],
                                'pit_stops': 2
                            })

        print(f"\nTotal strategies generated: {len(simulated_strategies)}")
